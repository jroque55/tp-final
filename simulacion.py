...
def proximo_pedido():
    ...
    T = TPP
    TPP = T + obtener_IE()
    CSB = max(int(obtener_PA() / 100),4)
    if(CSB <= BD):
        if(obtener_AL()):
            CLA = CLA +  1
            BD = BD - CSB
            SB = SB + (CSB * 50000) - math.ceil(CSB / 8) * CE
